...
func HclPort() (hcl, resourceName, resourceReferece string) {
	...
	pop, _, media, availableSpeed, err := GetPopAndZoneWithAvailablePort(os.Getenv(PF_CRC_SPEED_KEY))
	if err != nil {
		log.Panic(err)
	}
...
func HclCloudRouterConnectionAws() (hcl string, crResourceName, resourceName string) {
	...
	pop, _, _, _, err := GetPopAndZoneWithAvailablePort(os.Getenv(PF_CRC_SPEED_KEY))
	if err != nil {
		log.Fatal(err)
	}
	crcHcl := fmt.Sprintf(
